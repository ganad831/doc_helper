"""Delete field option command (Phase 2 Step 3).

Allows deleting options from choice fields.
"""
...
class DeleteFieldOptionCommand:
    """Delete option from choice field (DROPDOWN, RADIO).

    Phase 2 Step 3 Scope (Decision 5):
    - Delete existing options from choice fields
    - Validate field is a choice type
    - Validate option exists before deletion
    - Maintain option order (remaining options keep their relative order)

    Protected Invariants:
    - Only choice fields (DROPDOWN, RADIO) can have options
    - Option must exist in field before deletion
    - Options are immutable tuples (new tuple created on delete)
    """
    ...
    def execute(
        ...
        """Delete option from choice field.

        Args:
            entity_id: Parent entity ID (required)
            field_id: Field ID to delete option from (required)
            option_value: Option value to delete (identifier) (required)

        Returns:
            Result with FieldDefinitionId on success, error message on failure

        Validations:
            - entity_id, field_id, option_value must not be empty
        ...
        if not field_def.is_choice_field:
            return Failure(
                f"Cannot delete option from field type '{field_def.field_type.value}'. "
        ...
        if option_value_stripped not in existing_values:
            return Failure(
                ...
                "Cannot delete non-existent option."
            )

        # Create new options tuple without the deleted option
        updated_options = tuple(
            (value, label_key)
            for value, label_key in field_def.options
            if value != option_value_stripped
        )
        ...
        if save_result.is_failure():
            return Failure(f"Failed to save option deletion: {save_result.error}")
